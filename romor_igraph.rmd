...
# acc_i   个体是否接受romor,0为不接受，0.5为初始点，其他数字为接收时间点
...
get_p_send<-function(i,t,time_step){
  ...
  p<-exp(-1*id_all[i,1]*all_t)*sin(id_all[i,2]*all_t+id_all[i,3])
  return(p)
}
...
for(t in seq(time_step-10)){
  ...
  for(ii in spend_t){
    ...
    if(runif(1)<p_send[ii,t]){
      ...
      ra_ii<-which(acc_i>0)
      # 接受的次数
      acc_n[intersect(aci,ra_ii)]<-acc_n[intersect(aci,ra_ii)]+1
